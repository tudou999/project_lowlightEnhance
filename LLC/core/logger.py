...
def parse(args):
    ...
    if args.brutal_search:
        experiments_root = os.path.join(
            'experiments', opt['name'], '{}_noise_start:{}_noise_end:{}_{}'.format(opt['name'], args.noise_start, args.noise_end, get_timestamp()))
    else:
        if opt['phase'] == 'train':
            experiments_root = os.path.join(
                'experiments', opt['name'], '{}_w_snr:{}_w_str:{}_w_gt:{}_w_lpips:{}_{}'.format(opt['name'], args.w_snr, args.w_str, args.w_gt, args.w_lpips, get_timestamp()))
        elif opt['phase'] == 'test':
            experiments_root = os.path.join(
                './LLC/experiments', opt['name'], '{}_numstep:{}_w_snr:{}_w_gt:{}_w_lpips:{}_{}'.format(opt['name'],opt["model"]['beta_schedule']['val']['n_timestep'], args.w_snr, args.w_gt, args.w_lpips, get_timestamp()))
