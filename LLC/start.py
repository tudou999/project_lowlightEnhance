...
def run(
    ...
    source="../pic",  # file/dir/URL/glob/screen/0(webcam)
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if save_img:
                if dataset.mode == "image":
                    if 'results_np' not in locals():
                        results_np = []
                    results_np.append(im0.copy())
    ...
    if save_img and 'results_np' in locals():
        for idx, arr in enumerate(results_np):
            if hasattr(dataset, 'files'):
                img_name = Path(dataset.files[idx]).stem
            else:
                img_name = f"{idx+1}"
            npy_path = save_dir / f"{img_name}_det.npy"
            np.save(npy_path, arr)

def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument("--weights", nargs="+", type=str, default=ROOT / "../yolov5s.pt", help="model path or triton URL")
    parser.add_argument("--source", type=str, default="../pic", help="file/dir/URL/glob/screen/0(webcam)")
...
def main(opt):
    ...
    parser.add_argument('-c', '--config', type=str, default='./LLC/config/test_unpaired.json',
    ...
    parser.add_argument('--n_timestep', type=int, default=8)
    parser.add_argument('--w_str', type=float, default=0.)
    parser.add_argument('--w_snr', type=float, default=0.)
    parser.add_argument('--w_gt', type=float, default=0.)
    ...
    for i in range(len(Image_names)):
        ...
        raw_img = np.load(path, allow_pickle=True)  # 假设npy文件内容为HWC格式的uint8或float32
        if raw_img.dtype != np.float32:
            raw_img = raw_img.astype(np.float32) / 255.0  # 归一化到0-1
        img_h, img_w = raw_img.shape[:2]
        raw_img = torch.from_numpy(raw_img).permute(2, 0, 1)  # HWC->CHW
        raw_img = transforms.Resize((img_h // 16 * 16, img_w // 16 * 16))(raw_img)
        raw_img = raw_img.unsqueeze(0).cuda()
        ...
        niqe_scores = niqe(normal_img)
        ave_niqe = ave_niqe + niqe_scores
        llie_img_mode = 'single'
        if llie_img_mode == 'single':
            # util.save_img(ll_img, '{}/{}_input.png'.format(result_path, idx))
            util.save_img(
        ...
        logger.info('CNIQE: {} on {}'.format(niqe_scores, Image_names[i]))
    ave_niqe = ave_niqe / len(Image_names)
    logger.info('NIQE: {} on {}'.format(ave_niqe, InputPath))

    opt.source = result_path
